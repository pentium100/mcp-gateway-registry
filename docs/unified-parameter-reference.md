...
| Client-facing proxy URL prefix (registry) | `GATEWAY_PROXY_PREFIX` | `gateway_proxy_prefix` | `registry.app.gatewayProxyPrefix` | URL path segment that namespaces every auto-derived client route (`/{prefix}/{entity_type}/{name}`). Operators supply only the backend origin; the client path is derived. Rendered verbatim into an nginx `location`, so it is restricted to a single URL-safe segment (letters, digits, hyphen, underscore). Default `gateway`. |
